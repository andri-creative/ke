...
export const categoryService = {
    async getCategories(): Promise<Category[]> {
        ...
        return (Array.isArray(data) ? data : []).map((item: any) => ({
            ...item,
            status: item.isActive !== undefined ? item.isActive : (item.is_active !== undefined ? item.is_active : item.status)
        }));
    },

    async createCategory(payload: { name: string, type: string, status: boolean }): Promise<Category> {
        const token = localStorage.getItem(TOKEN_KEY);
        if (!token) {
            throw new Error('No token');
        }
